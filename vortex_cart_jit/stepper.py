...
def make_step(cfg):
    ...
    def step(state):
        ...
        pres_new = presi
        presi_p = edge_damp(0.5 * (pres + presi) + pdot * dt)
        vort_new = vorti
        vorti_p = edge_damp(0.5 * (vort + vorti) + wdot * dt)
        ...
        presi_c = edge_damp(pres_new + pdot_p * dt)
        vorti_c = edge_damp(vort_new + wdot_p * dt)
